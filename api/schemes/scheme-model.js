...
async function findById(scheme_id) {
  ...
  if (tasks) {
    const finalObj = {
      scheme_id: tasks[0].scheme_id,
      scheme_name: tasks[0].scheme_name,
      steps: tasks[0].step_number
        ? tasks.map((task) => ({
...
async function findSteps(scheme_id) {
  ...
  */
  ...
  return steps[0].step_number ? steps : [];
}
...
async function add(scheme) {
  ...
  */
  ...
  scheme = await findById(scheme_id);
  console.log("🚀 ~ file: scheme-model.js ~ line 168 ~ add ~ scheme", scheme);
